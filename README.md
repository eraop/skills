...
如果你只是想把现有 skill 安装到本机 agent：

```bash
npm install
npm run build
npm exec -- skills list
npm exec -- skills install code-generation-guardrails
```

`skills install <skill-name>` 默认会：
...
```bash
npm run build
npm exec -- skills build code-generation-guardrails
```
...
```bash
npm exec -- skills install code-generation-guardrails --target codex --scope project
```
...
```bash
npm exec -- skills build <skill-name>
npm exec -- skills install <skill-name> --target codex
```
...
# 安装某个 skill
npm exec -- skills install code-generation-guardrails --target codex --scope global
...
3. 运行 `npm exec -- skills build <skill-name>` 生成平台产物。
4. 到 `dist/<platform>/<skill-name>/` 查看每个 agent 实际会读取的文件。
5. 运行 `npm exec -- skills install <skill-name> --target <platform>` 安装。
...
- 再运行 `npm exec -- skills build <skill-name>`
