export function renderSkillEditor() {
  return `
    <section class="mt-6 rounded-lg border border-white/10 bg-black/25 p-5 shadow-2xl shadow-black/20">
      <label class="font-mono text-xs font-semibold text-copper" for="skill-source">Paste a complete skill</label>
      <textarea class="mt-4 min-h-[28rem] w-full resize-y border border-white/10 bg-ink/80 p-4 font-mono text-sm leading-6 text-porcelain outline-none transition placeholder:text-mist/60 focus:border-brass" id="skill-source" rows="18" spellcheck="false"></textarea>
      ...
      <button class="mt-5 inline-flex h-10 items-center justify-center border border-brass/50 bg-brass px-4 text-sm font-semibold text-ink transition hover:bg-porcelain focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brass disabled:hover:bg-brass" id="save-skill" type="button">Save and Build</button>
